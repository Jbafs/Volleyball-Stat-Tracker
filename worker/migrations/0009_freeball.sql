-- Add freeball_result column to rally_actions to track whether a freeball/overpass
-- went cleanly over ('over') or was an error ('error').
-- 'overpass' already exists in the action_type CHECK constraint, so the only
-- schema change is the new column. SQLite requires a table recreation to add it.
...
CREATE TABLE rally_actions_new (
  ...
  action_type       TEXT NOT NULL CHECK(action_type IN (
                      'serve','pass','set','attack','dig','block','overpass'
                    )),
